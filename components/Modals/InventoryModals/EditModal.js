...
const EditModal = (props) => {
  useEffect(() => {
    if (props.selectedItem) {
      ...
      setType(props.selectedItem.category.id);
      setPrice(props.selectedItem.unit_price);
      setCost(props.selectedItem.cost);
      setQuantity(props.selectedItem.stock);
      setDescription(props.selectedItem.description);
      props.selectedItem.images.map((item, index) => {
        if (index === 0) {
          setFirstPhoto(item.image);
          setDisplayFirst(item.image);
        }
        if (index === 1) {
          setSecondPhoto(item.image);
          setDisplaySecond(item.image);
        }
        if (index === 2) {
          setThirdPhoto(item.image);
          setDisplayThird(item.image);
        }
      });
    }
  }, [props.selectedItem]);
  ...
  const [displaySecond, setDisplaySecond] = useState(null);
  const [displayThird, setDisplayThird] = useState(null);
  ...
  const handleSubmit = (event) => {
    ...
    if (error) {
      ...
      if (props.selectedItem.images[0] !== firstPhoto && firstPhoto != null) {
        form_data.append('images', firstPhoto);
      } else {
        if (props.selectedItem.images[0])
          form_data.append('delete_keys', props.selectedItem.images[0].uuid);
      }
      if (props.selectedItem.images[1] !== secondPhoto && secondPhoto != null)
        form_data.append('images', secondPhoto);
      else {
        if (props.selectedItem.images[1])
          form_data.append('delete_keys', props.selectedItem.images[1].uuid);
      }

      if (props.selectedItem.images[2] !== thirdPhoto && thirdPhoto != null)
        form_data.append('images', thirdPhoto);
      else {
        if (props.selectedItem.images[2])
          form_data.append('delete_keys', props.selectedItem.images[2].uuid);
      }
  ...
  const closeModal = () => {
    ...
    setDisplayThird(null);
    setDisplaySecond(null);
    setDisplayFirst(null);
    setSecondPhoto(null);
    setThirdPhoto(null);
    setFirstPhoto(null);
  };
